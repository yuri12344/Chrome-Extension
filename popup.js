...
var settingsMoreContainer = $('#settings-more');
...
function populateProxyList() { //[populateProxyList]
  ...
  function onProxiesReady(proxies, some_options) {
    ...
    if (proxies.length) {
      ...
      divSelectList.appendTo(settingsMoreContainer);
      ...
      divPreventLeakDiv.appendTo(settingsMoreContainer);
      ...
      var parent_more = $('<div>').addClass('parent_more').appendTo(settingsMoreContainer)
      .hover(
        function() { // mouse in
          btn_more.hide();
          btn_enter.show();
      ...
      .on('click', function (e) {
       ...
       /* settingsMoreContainer.transition({
          rotateY: '+=90deg',
          duration: anim_duration,
          easing: 'in',
          complete: function() {
            settingsMoreContainer.hide();
          }
        });	*/	
...
$(document).ready(function() {
      ...
      complete: function() {
        ...
        proxyListContainer.transition({
          ...
          complete: function() {
            ...
            proxyListContainer.css('transform', 'none');
			//settingsMoreContainer.show();
  ...
  .on('click', function (e) {
    if (!$(this).hasClass('disabled')) {
		...
		if ((isActivated) && ($('#textarea_key').val()==="")){
			//SyncStorage.saveKey("");
			ChromeStorage.clearAll();
			isActivated = false;
			setStatusMessage("key removed", false);
			Reload();
